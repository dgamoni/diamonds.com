...
add_filter( 'woocommerce_gallery_image_html_attachment_image_params', 'filter_function_name_8463', 10, 4 );
function filter_function_name_8463( $array, $attachment_id, $image_size, $main_image ){
	unset($array['width']);
	unset($array['height']);
	return $array;
}
...
/** custom function for the RAPNET. 12.04.2019 (Version: 1.0.0) */

remove_action( 'wp_ajax_nopriv_rapnet_add_cart', 'nwg_rapnet_add_cart_callback' );
remove_action( 'wp_ajax_rapnet_add_cart', 'nwg_rapnet_add_cart_callback' );
add_action( 'wp_ajax_nopriv_rapnet_add_cart', 'nwg_rapnet_add_cart_custom_callback' );
add_action( 'wp_ajax_rapnet_add_cart', 'nwg_rapnet_add_cart_custom_callback' );

function nwg_rapnet_add_cart_custom_callback() {
    $response = array(
           'message'=>'Unable to Add Diamond to Cart'
    );
    $header = 400;

    if(isset($_POST['data'])) {
		
		/** $_POST data example (not all parameters): 
		array (
		  'data' => 
		  array (
			'diamond' => 
			array (
			  ...
			  'city' => 'Ramat Gan',
			  'lab' => 'GIA',
			  'total_purchase_price' => '3671.35',
			  'total_sales_price' => '4038',
			  'currency_code' => 'USD',
			  'currency_symbol' => '$',
			  'total_sales_price_in_currency' => '4038',
			  'cert_num' => '14836703',
			  'stock_num' => 'GM-0208',
			...
			array (
			  'account_id' => '54505',
			  'company' => 'M.I.D House Of Diamonds (Los Angeles)',
			  'name' => 'Dotan Meirov',
			  'email' => 'info@middiamonds.com',
			  'phone' => '',
			  'country' => 'USA',
			  'state' => 'CA',
			  'city' => 'Los Angeles',
		  ...
		  'action' => 'rapnet_add_cart',
		  'req_url' => '/ring-builder-diamond-select/?type=diamond',
		  'flow' => '',
		)
		*/
        $options = get_option('rapnet_options');        
        global $woocommerce;

        $diamond = $_POST['data']['diamond'];
        $seller = $_POST['data']['seller'];
		$message .= '<br><p>';		
		if (isset($diamond['diamond_id']) && $diamond['diamond_id']) $message.= '• Diamond ID: ' . $diamond['diamond_id'] . '<br>';
		if (isset($diamond['shape']) && $diamond['shape']) $message.= '• Shape: ' . $diamond['shape'] . '<br>';
		if (isset($diamond['size']) && $diamond['size']) $message.= '• Size: ' . $diamond['size'] . '<br>';
		if (isset($diamond['color']) && $diamond['color']) $message.= '• Color: ' . $diamond['color'] . '<br>';
		if (isset($diamond['clarity']) && $diamond['clarity']) $message.= '• Clarity: ' . $diamond['clarity'] . '<br>';
		if (isset($diamond['cut']) && $diamond['cut']) $message.= '• Cut: ' . $diamond['cut'] . '<br>';
		if (isset($diamond['symmetry']) && $diamond['symmetry']) $message.= '• Symmetry: ' . $diamond['symmetry'] . '<br>';
		if (isset($diamond['polish']) && $diamond['polish']) $message.= '• Polish: ' . $diamond['polish'] . '<br>';
		if (isset($diamond['lab']) && $diamond['lab']) $message.= '• Lab: ' . $diamond['lab'] . '<br>';
		if (isset($diamond['cert_num']) && $diamond['cert_num']) $message.= '• Certificate Number: ' . $diamond['cert_num'] . '<br>';
		if (isset($diamond['girdle_min']) && $diamond['girdle_min']) $message.= '• Minimal Girdle: ' . $diamond['girdle_min'] . '<br>';
		if (isset($diamond['girdle_max']) && $diamond['girdle_max']) $message.= '• Maximum Girdle: ' . $diamond['girdle_max'] . '<br>';
		if (isset($diamond['culet_size']) && $diamond['culet_size']) $message.= '• Culet Size: ' . $diamond['culet_size'] . '<br>';
		if (isset($diamond['fluor_intensity']) && $diamond['fluor_intensity']) $message.= '• Fluor Intensity: ' . $diamond['fluor_intensity'] . '<br>';
		if (isset($diamond['depth_percent']) && $diamond['depth_percent']) $message.= '• Depth Percent: ' . $diamond['depth_percent'] . '<br>';
		if (isset($diamond['table_percent']) && $diamond['table_percent']) $message.= '• Table Percent: ' . $diamond['table_percent'] . '<br>';
		if (isset($diamond['meas_length']) && $diamond['meas_length']) $message.= '• Meas Length: ' . $diamond['meas_length'] . '<br>';
		if (isset($diamond['meas_width']) && $diamond['meas_width']) $message.= '• Meas Width: ' . $diamond['meas_width'] . '<br>';
		if (isset($diamond['meas_depth']) && $diamond['meas_depth']) $message.= '• Meas Depth: ' . $diamond['meas_depth'] . '<br>';
		$message .= '</p>';

        //price
        $invoiceprice = $diamond['total_sales_price_in_currency'];

        //Generate title
        $title = $diamond["size"]. "-CARAT ".strtoupper($diamond["shape"]). " DIAMOND";
        ...
        if($post_id){
            ...
            $color = str_replace("#", "", $options['theme']);
            $filename = nwg_generate_image($diamond['shape'], $color);
            ...
            $product_id        = apply_filters( 'woocommerce_add_to_cart_product_id', absint( $post_id ) );
            ...
            if ( $passed_validation && WC()->cart->add_to_cart( $product_id, $quantity, $variation_id, $variation ) ) {
                ...
                if(strstr($_POST['req_url'], 'ring-builder-diamond-select')) {
                    if($_POST['flow']==1) {
                        $response['url'] = '/ring-builder-diamond-select/?type=summary&flow=1';
                    } else {
                        $response['url'] = '/product-category/bridal/select-a-setting/?added=1';
                    }
                }
                $header = 200;
...
// save acf in files - need for easy transfer from staging to live by hands
...
// add gravvy functions

add_action('woocommerce_after_order_notes', 'woo_gravvy_html');

function woo_gravvy_html($checkout) {
	wc_get_template( 'checkout/form-engraving.php', array( 'checkout' => $checkout ) );
}
...
// ajaxs
if (!function_exists('ajax_simply_enqueue_scripts') ) require_once('inc/ajax-simply.php');

// ajax for the upload the font 
function ajaxs_engraving_upload_font ($data) {
	$response = array();
	
	if (isset($_FILES['custom_engraving_file'])) {
		require_once ABSPATH . 'wp-admin/includes/image.php';
		require_once ABSPATH . 'wp-admin/includes/file.php';
		require_once ABSPATH . 'wp-admin/includes/media.php';

		$movefile = media_handle_upload( 'custom_engraving_file', 0);
		
		if ( is_wp_error( $movefile ) ) {
			$response['error'] = $error->get_error_message();
		} else {
			$response['error'] = '';
			$response['url'] = wp_get_attachment_url($movefile);
			$response['name'] = $_FILES['custom_engraving_file']['name'];
		}
	} else {
...
function save_engraving_settings ($posted_data) {
	...
	if (isset($post['engraving_text'])){
		...
		if (WC()->cart->get_cart_contents()) {
			foreach(WC()->cart->get_cart_contents() as $item) {
				if ($item['product_id'] == $engraving_product->ID) $engraving_key = $item['key'];
			}
		}

		if ($post['engraving_text']) {
			
			// check product 
			if (!$engraving_key) {
				// add product 
				$engraving_key = WC()->cart->add_to_cart($engraving_product->ID);
			}
			
			if (!$engraving_key) return; // something wrong happens!
			
			// change settings 
			
			// first font choose checkboxes 
			WC()->cart->cart_contents[ $engraving_key ]['font'] = $post['engraving_font_choose'];
			
			// font family
			if ($post['engraving_font_choose'] == 'print') {
				WC()->cart->cart_contents[ $engraving_key ]['font-family'] = 'Print';
			}
			
			if ($post['engraving_font_choose'] == 'cursive') {
				WC()->cart->cart_contents[ $engraving_key ]['font-family'] = 'Cursive';
			}
			...
			WC()->cart->cart_contents[ $engraving_key ]['file-link'] = $post['custom_engraving_font_url'];
			WC()->cart->cart_contents[ $engraving_key ]['inscription'] = $post['engraving_text'];
			
		} else {
			// delete product
			if ($engraving_key) WC()->cart->remove_cart_item($engraving_key);
		}
...
add_action('woocommerce_checkout_update_order_review', 'save_engraving_settings');

function iconic_display_engraving_text_cart( $item_data, $cart_item ) {
	
	if (isset($cart_item['font-family'])) {
		
		$font = '';
		
		if ($cart_item['use-file']) {
			$font = '<a href="'.$cart_item['file-link'].'" class="font-link">'.$cart_item['font-family'].'</a>';
		} else {
			$font = wc_clean( $cart_item['font-family'] );
		}
...
function iconic_add_engraving_text_to_order_items2( $item, $cart_item_key, $values, $order ) {
	
    if ( empty( $values['font-family'] ) ) {
        return;
    }
	...
	if ($values['use-file']) {
		$font = '<a href="'.$values['file-link'].'" class="font-link">'.$values['font-family'].'</a>';
	} else {
		$font = wc_clean( $values['font-family'] );
	}
		
	$item->add_meta_data( 'Font', $font );
	$item->add_meta_data( 'Text', $values['inscription'] );    
